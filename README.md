...
This project includes optional Docker support for those who want to containerize their application. This is particularly useful if you plan to deploy to AWS ECS or EC2.
...
1. **AWS ECS/EC2 Deployment**
   - Copy `.github/workflows/docker-deploy.example.yml` to `.github/workflows/docker-deploy.yml`
   - Uncomment and configure the ECR and ECS deployment steps
   - Set up the required AWS credentials and repository names

2. **Local Development**
   - Use the development target for local development
   - Mount your source code as a volume for live reloading
   ```bash
   docker run -p 5173:5173 -v $(pwd):/app app:dev
   ```
...
Note: Docker support is optional and not required for the basic S3/CloudFront deployment setup.
